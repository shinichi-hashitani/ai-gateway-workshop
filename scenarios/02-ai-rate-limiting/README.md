...
あわせて `ai-proxy-advanced` の `weighted-round-robin` で安価なモデルを優先します。

```
クライアント → Kong → OpenAI
                ├── ai-rate-limiting-advanced  (1分 2,000 トークンまで → 超過で 429)
                └── ai-proxy-advanced          (gpt-4o-mini 90% / gpt-4o 10%)
```
...
| プラグイン | 役割 |
|-----------|------|
| `ai-proxy-advanced` | gpt-4o-mini (weight 90) / gpt-4o (weight 10) で振り分け |
| `ai-rate-limiting-advanced` | Consumer ごとに 1分間 2,000 トークンまで |
| `key-auth` | クライアント認証 |
...
HTTP/2 200
X-Kong-AI-Tokens-Consumed: 42
RateLimit-Remaining: 1958
...
HTTP/2 200
RateLimit-Limit: 2000
RateLimit-Remaining: 1834
RateLimit-Reset: 38
X-Kong-AI-Tokens-Consumed: 166
```
...
`weight: 90` の gpt-4o-mini と `weight: 10` の gpt-4o に 9:1 で振り分けます。  
